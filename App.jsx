...
function buildWordHTML(entityName, entitySub, sections, footerNote=""){
  ...
  const tableSections = sections.map(sec=>`
    <tr><td colspan="4" style="background:#1e3f87;color:#fff;font-weight:bold;padding:7px 12px;font-size:11pt;letter-spacing:0.04em">${esc(sec.dayLabel)}</td></tr>
    ...
    ${sec.rows.map((r,i)=>`<tr style="background:${i%2===0?"#f3f5fb":"#fff"}">
      ...
      <td style="padding:8px 10px;vertical-align:top"><strong style="font-size:11pt">${esc(r.col1)}</strong>${r.col1b?`<br/><span style="font-size:9pt;color:#666">${esc(r.col1b)}</span>`:""}${r.col1c?`<br/><span style="font-size:9pt;color:#555;font-style:italic">${esc(r.col1c)}</span>`:""}</td>
      <td style="padding:8px 10px;vertical-align:top;font-size:10pt">${esc(r.col2)}</td>
      <td style="padding:8px 10px;vertical-align:top;font-style:italic;font-size:10pt">${esc(r.col3)}</td>
    </tr>`).join("")}
  `).join("");
...
function buildPrintHTML(entities, options={}){
  // entities: [{name, sub, sections}]  — same structure as buildWordHTML
  const entityHTMLs = entities.map((e,ei)=>`
    <div class="entity" ${ei>0?'style="page-break-before:always"':''}>
      <div class="ls-hdr">
        <div class="ls-logo">LATIN<br>SECURITIES</div>
        <div class="ev-info"><div style="font-size:13pt;font-weight:700;color:#1e3f87">Argentina in New York 2026</div><div style="font-size:9pt;color:#666">Investment Conference &middot; April 14&ndash;15, 2026</div></div>
      </div>
      <h1>${esc(e.name)}</h1>
      <h2>${esc(e.sub)}</h2>
      <table>${e.sections.map(sec=>`
        <tr><td colspan="4" class="day-hdr">${esc(sec.dayLabel)}</td></tr>
        <tr class="tbl-hdr">${sec.headerCols.map(h=>`<th>${esc(h)}</th>`).join("")}</tr>
        ${sec.rows.map((r,i)=>`<tr class="${i%2===0?"even":"odd"}">
          <td class="t-time">${esc(r.time)}</td>
          <td><strong>${esc(r.col1)}</strong>${r.col1b?`<br/><small>${esc(r.col1b)}</small>`:""}${r.col1c?`<br/><em>${esc(r.col1c)}</em>`:""}</td>
          <td>${esc(r.col2)}</td>
          <td class="t-room">${esc(r.col3)}</td>
        </tr>`).join("")}
      `).join("")}</table>
      ${e.attendees?.length?`<div class="attendees"><strong>Company Representatives:</strong> ${e.attendees.map(a=>`${esc(a.name)}${a.title?` (${esc(a.title)})`:""}`).join(" &bull; ")}</div>`:""}
    </div>
  `).join("");
...
.entity{max-width:820px;margin:0 auto}
...
.even td{background:#f3f5fb}.odd td{background:#fff}
...
small{font-size:9pt;color:#666}.ev-info{text-align:right}
.attendees{font-size:9.5pt;color:#555;margin-top:8px;padding-top:8px;border-top:1px dashed #dde}
@media print{body{padding:0}.entity{page-break-before:always}
  .entity:first-child{page-break-before:avoid}
...
</style></head><body>${entityHTMLs}</body></html>`;
}
...
function investorToExportData(inv, meetings, companies){
  ...
  const sections = DAYS.filter(d=>dayGroups[d]).map(d=>({
    dayLabel: DAY_LONG[d],
    headerCols:["Time","Company","Sector","Room"],
    rows: dayGroups[d].map(m=>{
      const co=companies.find(c=>c.id===m.coId);
      return {time:hourLabel(slotHour(m.slotId)),col1:co?.name||m.coId,col1b:co?.ticker,col1c:null,col2:co?.sector||"",col3:m.room};
    })
  }));
  return {
    name: inv.name,
